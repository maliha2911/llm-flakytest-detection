...
def parse_args():
    parser = argparse.ArgumentParser(description="QLoRA fine-tuning of Mistral 7B for flaky test classification.")
    parser.add_argument("--data", required=True, help="Path to flaky_data.xlsx")
    parser.add_argument("--model-path", required=True, help="Local path or HF ID of base Mistral model")
    ...
    parser.add_argument("--output-dir", default="./mistral_flaky_qlora", help="Where to save LoRA adapter")
...
def main():
    ...
    print("=" * 80)
    print("QLoRA fine-tuning Mistral 7B for flaky test classification")
    print("=" * 80)
    ...
    # 5) Load model in 4-bit and prepare for k-bit training
    ...
    # 6) LoRA config (QLoRA still uses LoRA adapters)
    ...
    # 8) TrainingArguments (no fp16/bf16 flags needed; handled by 4-bit config)
    ...
    print("\nSaved QLoRA adapter + tokenizer to:", output_dir)
