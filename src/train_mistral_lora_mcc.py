...
# ---------- Reuse pieces from your existing script ----------
...
# ---------- Improved LoRA fine-tuning script (MCC-oriented) ----------

def parse_args():
    parser = argparse.ArgumentParser(description="LoRA fine-tuning of Mistral 7B for flaky test classification (MCC-optimized).")
    parser.add_argument("--data", required=True, help="Path to flaky_data.xlsx")
    parser.add_argument("--model-path", required=True, help="Local path to base Mistral model folder")
    ...
    parser.add_argument("--random-state", type=int, default=66, help="Random seed for splits")
    parser.add_argument("--output-dir", default="./mistral_flaky_lora_mcc", help="Where to save LoRA adapter")
    parser.add_argument("--max-length", type=int, default=1024, help="Max sequence length")
    parser.add_argument("--per-device-train-batch-size", type=int, default=1)
    parser.add_argument("--per-device-eval-batch-size", type=int, default=1)
    parser.add_argument("--gradient-accumulation-steps", type=int, default=8)
    parser.add_argument("--num-train-epochs", type=float, default=5.0)  # longer training
    parser.add_argument("--learning-rate", type=float, default=1e-4)    # smaller LR
    parser.add_argument("--weight-decay", type=float, default=0.01)     # mild regularization
    parser.add_argument("--warmup-ratio", type=float, default=0.05)     # smoother warmup
...
def balance_classes_for_mcc(df: pd.DataFrame, random_state: int) -> pd.DataFrame:
    ...
    This tends to improve MCC by reducing class imbalance.
    """
    counts = df["label"].value_counts()
    if len(counts) != 2:
        # If something weird happens, just return as-is
        return df
...
def main():
    ...
    print("=" * 80)
    print("LoRA fine-tuning Mistral 7B for flaky test classification (MCC-optimized)")
    print("=" * 80)
    ...
    # 1.1) Class balancing to improve MCC
    ...
    # 2) Build training text (prompt + gold label)
    ...
    # 6) TrainingArguments (tuned for better MCC via cleaner boundary & regularization)
